...
class TestRepoList(object):
    ...
    def test_filtered_repo_list(self, mocked_get):
        for repo in clone_army.repositories('18F', filter=r'\.'):
            assert '.' in repo['name']
...
class TestCloning(object):
    ...
    def test_args_passed_when_cloning(self, mocked_get, mocked_run):
        ...
        repo1.synch('--depth', '1')
        mocked_run.assert_called_with(['git', 'clone', '--depth', '1', 'https://github.com/18F/14c-prototype.git'])

    @mock.patch('os.path.exists', return_value=True)
    def test_pulls_when_dir_exists(self, mocked_exists, mocked_get, mocked_run):
        repos = clone_army.repositories('18F')
        repo1 = clone_army.Repository(repos.__next__())
        repo1.synch()
